...
Deno.serve(async (req) => {
  try {
    ...
    const payerText = ['✅ Invoice paid', '', amountLine, `Paid to: @${sellerUsername}`, `Memo: ${clean(invoice.memo, '-')}`, '', `Paid at: ${paidAt}`, `Tx: ${shortTx}`, 'Verified by Arqis backend', '', 'Receipt:', 'https://www.arqis.site/pay-invoice'].join('\n');
    const sellerText = ['💸 Payment received', '', amountLine, `Paid by: @${payerUsername}`, `Memo: ${clean(invoice.memo, '-')}`, '', `Paid at: ${paidAt}`, `Tx: ${shortTx}`, 'Verified by Arqis backend', '', 'Open Seller Console:', 'https://www.arqis.site/#tab-console'].join('\n');
    const results = [];
